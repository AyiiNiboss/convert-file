...
const JpgToPdfLayout = () => {
  ...
  const handleDrop = (e) => {
    ...
    if (dropFile && dropFile.type === "image/jpeg") {
      setFile(dropFile);
    } else {
      setFile(null);
      handleErrorUpload();
  ...
  const handleConvertPdfToWord = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://v2.convertapi.com/convert/jpg/to/pdf?Secret=1S95roLYF5Hdfy0R",
        formData,
        {
          headers: {
  ...
  return (
    <>
      <div className="bg-slate-200 text-slate-800 font-mono w-full min-h-screen flex flex-col items-center justify-center gap-5">
        ...
        <div>
          <h5 className="text-4xl font-semibold">Konversi JPG ke PDF</h5>
        </div>
