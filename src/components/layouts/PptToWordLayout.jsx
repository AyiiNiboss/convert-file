...
const PptToPdfLayout = () => {
  ...
  const handleDrop = (e) => {
    ...
    if (dropFile && dropFile.type === "application/vnd.openxmlformats-officedocument.presentationml.presentation") {
      setFile(dropFile);
    } else {
      setFile(null);
      handleErrorUpload();
  ...
  const handleConvertPdfToWord = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://v2.convertapi.com/convert/pptx/to/pdf?Secret=1S95roLYF5Hdfy0R",
        formData,
        {
          headers: {
  ...
  return (
    <div className="bg-slate-200 text-slate-800 font-mono w-full min-h-screen flex flex-col items-center justify-center gap-5">
      ...
      <div>
        <h5 className="text-4xl font-semibold">Konversi Powerpoint ke PDF</h5>
      </div>
      <div
